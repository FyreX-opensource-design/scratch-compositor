...
| **`zwlr_screencopy_manager_v1`** (wlr-screencopy-unstable) | `wlr_screencopy_manager_v1_create(dpy)` | Screen capture for clients that use the wlr screencopy protocol (e.g. **grim**, some recorder tools). Works with this compositor’s **`wlr_scene_output`** commit path. |
...
| **`ext_workspace_manager_v1`** (staging **ext-workspace-v1**) | `wl_global_create` + `wayland-scanner` server code in `src/ext_workspace.c` | **Version 1**. Nine fixed workspaces (**`1`**..**`9`**), one **workspace group** spanning all **`wl_output`** bindings for that client; stable ids `stackcomp-ws-0`..`stackcomp-ws-8`; **`activate`** switches the current workspace (same as keybinds / IPC). After workspace switches, emits updated **`state`** on each **`ext_workspace_handle_v1`**, then **`done`** on the manager (required for atomic client updates; e.g. **waybar** **`ext/workspaces`** active styling). No **`create_workspace`**, **`remove`**, **`assign`**, or **`deactivate`** support. |
| **`zxdg_decoration_manager_v1`** (xdg-decoration-unstable-v1) | `wlr_xdg_decoration_manager_v1_create(dpy)` | Lets clients negotiate **client-side** vs **server-side** decorations. Stackcomp sets **server-side** in tile/scroll (non-float) when config says to strip client title bars (no SSD is drawn, so surfaces look borderless); **client-side** in stack layout and for tile-float windows. |
| **`zwlr_layer_shell_v1`** (wlr-layer-shell unstable) | `wlr_layer_shell_v1_create(dpy, 4)` | Version **4**. Desktop “shell” surfaces (panels, wallpapers, overlays) via **`wlr_scene_layer_surface_v1`**. Scene order (back → front): **background**, **output layout**, **bottom**, **windows**, **top**, **overlay** (bottom panels sit above the desktop but below XDG windows). **`wlr_scene_layer_surface_v1_configure`** drives exclusive zones; each output’s **`layer_workarea`** is the tiling / scroll region for windows whose center falls in that workarea (multi-head: independent grids / scroll columns per physical output). **Stack** placement still uses the output under the pointer (or a fallback). **Keyboard:** pointer click on a layer surface can move seat keyboard focus for **on-demand** and (top/overlay) **exclusive** interactivity; otherwise keyboard stays on the focused XDG toplevel. |
...
**Not created anywhere in this repo:** foreign toplevel management, output management, export-dmabuf (for portal capture), gamma, idle inhibit, presentation time, virtual keyboard/pointer, tablet/touch protocol objects on the seat, etc.
...
- **`COMPOSITOR.md`** — feature scope and build (including **workspaces**: nine virtual desktops, not a separate Wayland protocol).
- **`CONFIG.md`** — keybinds, **`when=`** shell env (**`STACKCOMP_LAYOUT`**, **`STACKCOMP_WORKSPACE`**), and **IPC** lines such as **`workspace N`** / **`workspace move N`**.
